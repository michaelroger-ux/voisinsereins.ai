    ...
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:`linear-gradient(170deg,${T.warmWhite} 0%,${T.sand} 40%,${T.leafLight}33 100%)`,fontFamily:SANS,overflow:"auto",position:"relative"}}>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"30px 28px 20px",position:"relative",zIndex:1,opacity:show?1:0,transform:show?"none":"translateY(20px)",transition:"all 0.8s cubic-bezier(0.16,1,0.3,1)"}}>
        ...
        <div style={{width:"100%",maxWidth:320,padding:14,borderRadius:16,background:`${T.purple}08`,border:`1.5px solid ${T.purple}25`}}>
          <div style={{fontSize:10,fontWeight:700,color:T.purple,textTransform:"uppercase",letterSpacing:1,marginBottom:10,display:"flex",alignItems:"center",gap:5}}>
            <span style={{fontSize:12}}>🧪</span> Simuler un scénario d'entrée
          </div>
          {[
            ...
            <button key={m.id} onClick={()=>setSimMode(m.id)} style={{
              ...
              <div style={{flex:1}}>
                <div style={{fontSize:11,fontWeight:600,color:simMode===m.id?T.purple:T.text}}>{m.label}</div>
                <div style={{fontSize:9,color:T.textMuted}}>{m.desc}</div>
              </div>
              {simMode===m.id&&<span style={{color:T.purple,fontSize:14,fontWeight:700}}>✓</span>}
            </button>
          ))}
          {simMode&&<Btn full onClick={()=>onNext(simMode)} style={{marginTop:6,background:`linear-gradient(135deg,${T.purple},${T.sky})`}}>
            🚀 Lancer le scénario {simMode==="invitation"?"A":simMode==="qr_generic"?"B.1":simMode==="qr_copro"?"B.2":"C"}
          </Btn>}
        </div>
        <p style={{fontSize:10,color:T.textMuted,textAlign:"center",marginTop:16,lineHeight:1.5,maxWidth:260}}>Dans la vraie app, cet écran n'existe pas — l'utilisateur arrive directement selon son contexte.</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:T.warmWhite,fontFamily:SANS}}>
      <div style={{padding:"56px 24px 20px",background:`linear-gradient(170deg,${T.forest},${T.forestLight})`,borderRadius:"0 0 28px 28px",flexShrink:0}}>
        ...
        <p style={{color:"rgba(255,255,255,0.7)",fontSize:13,margin:0}}>{subtitle||"Créez un compte ou connectez-vous"}</p>
      </div>
      <div style={{flex:1,padding:"24px 20px",overflowY:"auto",opacity:show?1:0,transition:"opacity 0.5s 0.3s"}}>
        ...
        {step==="main"&&<div>
          <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:20}}>
            {[{l:"Continuer avec Google",i:"G",bg:"#fff",c:T.text,b:`1px solid ${T.sandDark}`},{l:"Continuer avec Apple",i:"",bg:"#000",c:"#fff",b:"none"},{l:"Continuer avec Facebook",i:"f",bg:"#1877F2",c:"#fff",b:"none"}].map((btn,i)=>(
              <button key={i} onClick={()=>onAuth({method:btn.l,isNew:false})} style={{width:"100%",padding:"13px 20px",borderRadius:12,border:btn.b,background:btn.bg,color:btn.c,fontSize:14,fontWeight:600,fontFamily:SANS,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,boxShadow:"0 2px 8px rgba(0,0,0,0.06)"}}>
                <span style={{fontSize:16,fontWeight:700,width:20,textAlign:"center"}}>{btn.i}</span>{btn.l}
              </button>
            ))}
          </div>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
            <div style={{flex:1,height:1,background:T.sandDark}}/><span style={{fontSize:11,color:T.textMuted,fontWeight:500}}>ou par email</span><div style={{flex:1,height:1,background:T.sandDark}}/>
          </div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:10,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Adresse email</label>
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="jean@exemple.fr" autoFocus onKeyDown={e=>e.key==="Enter"&&checkEmail()} style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`2px solid ${email.includes("@")?T.forest:T.sandDark}`,fontSize:14,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box",transition:"border-color 0.3s"}}/>
          </div>
          <Btn full disabled={!email.trim()||!email.includes("@")} onClick={checkEmail}>Continuer →</Btn>
          <p style={{fontSize:10,color:T.textMuted,textAlign:"center",marginTop:14,lineHeight:1.5}}>Si vous avez déjà un compte, nous le retrouverons. Sinon, nous en créerons un.</p>
        </div>}
        ...
        {step==="password"&&<div>
          <div style={{padding:"12px 16px",background:`${T.leafLight}15`,borderRadius:12,marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
            ...
            <div><div style={{fontSize:13,fontWeight:600,color:T.forest}}>Content de vous revoir !</div><div style={{fontSize:11,color:T.textMuted}}>{email}</div></div>
          </div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:10,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Mot de passe</label>
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" autoFocus onKeyDown={e=>e.key==="Enter"&&password.trim()&&onAuth({method:"email",email,isNew:false})} style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`2px solid ${T.sandDark}`,fontSize:14,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box"}}/>
          </div>
          <Btn full disabled={!password.trim()} onClick={()=>onAuth({method:"email",email,isNew:false})}>Se connecter</Btn>
          <button onClick={()=>{setStep("main");setPassword("")}} style={{width:"100%",marginTop:10,padding:10,background:"transparent",border:"none",cursor:"pointer",fontFamily:SANS,fontSize:12,color:T.forestLight,fontWeight:500}}>← Changer d'email</button>
          <button style={{width:"100%",marginTop:4,padding:8,background:"transparent",border:"none",cursor:"pointer",fontFamily:SANS,fontSize:11,color:T.textMuted}}>Mot de passe oublié ?</button>
        </div>}

        {/* ── NEW ACCOUNT: choose password ── */}
        {step==="create_password"&&<div>
          <div style={{padding:"12px 16px",background:`${T.sky}10`,borderRadius:12,marginBottom:16,display:"flex",alignItems:"center",gap:10}}>
            <span style={{fontSize:20}}>🆕</span>
            <div><div style={{fontSize:13,fontWeight:600,color:T.sky}}>Bienvenue ! Créons votre compte</div><div style={{fontSize:11,color:T.textMuted}}>{email}</div></div>
          </div>
          <div style={{marginBottom:10}}>
            <label style={{fontSize:10,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Choisir un mot de passe</label>
            <input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="8 caractères minimum" autoFocus style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`2px solid ${password.length>=8?T.forest:T.sandDark}`,fontSize:14,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box",transition:"border-color 0.3s"}}/>
          </div>
          <div style={{marginBottom:14}}>
            <label style={{fontSize:10,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Confirmer</label>
            <input type="password" value={passwordConfirm} onChange={e=>setPasswordConfirm(e.target.value)} placeholder="••••••••" onKeyDown={e=>e.key==="Enter"&&password.length>=8&&password===passwordConfirm&&onAuth({method:"email",email,isNew:true})} style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`2px solid ${passwordConfirm&&password===passwordConfirm?T.forest:T.sandDark}`,fontSize:14,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box",transition:"border-color 0.3s"}}/>
            {passwordConfirm&&password!==passwordConfirm&&<p style={{fontSize:10,color:T.coral,margin:"4px 0 0"}}>Les mots de passe ne correspondent pas</p>}
          </div>
          ...
          <button onClick={()=>{setStep("main");setPassword("");setPasswordConfirm("")}} style={{width:"100%",marginTop:10,padding:10,background:"transparent",border:"none",cursor:"pointer",fontFamily:SANS,fontSize:12,color:T.forestLight,fontWeight:500}}>← Changer d'email</button>
        </div>}
      </div>
      {onCancel&&<div style={{padding:"8px 20px 16px",flexShrink:0}}>
        <button onClick={onCancel} style={{width:"100%",padding:10,background:"transparent",border:"none",cursor:"pointer",fontFamily:SANS,fontSize:13,color:T.textMuted,fontWeight:500,textAlign:"center"}}>← Revenir à l'accueil</button>
      </div>}
    </div>
  );
}
  ...
  return (
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:T.warmWhite,fontFamily:SANS}}>
      <div style={{padding:"56px 24px 18px",background:`linear-gradient(170deg,${T.forest},${T.forestLight})`,borderRadius:"0 0 28px 28px",flexShrink:0}}>
        <p style={{color:T.leafLight,fontSize:12,fontWeight:600,letterSpacing:"1.5px",textTransform:"uppercase",margin:"0 0 6px"}}>{stepLabel}</p>
      ...
      <div style={{padding:"16px 16px 0",flex:1,overflowY:"auto",minHeight:0}}>
        ...
        {entryMode==="invitation"&&<div style={{padding:"10px 14px",borderRadius:12,background:`${T.sky}10`,border:`1.5px solid ${T.sky}30`,marginBottom:12,display:"flex",alignItems:"center",gap:10,animation:"fadeIn 0.5s ease"}}>
          <span style={{fontSize:20}}>📨</span>
          <div><div style={{fontSize:12,fontWeight:600,color:T.sky}}>Invitation reçue</div><div style={{fontSize:10,color:T.textMuted}}>Un voisin vous a invité à rejoindre <strong>{copro?.name}</strong>. {copro?.members||0} résidents déjà actifs.</div></div>
        </div>}
        {entryMode==="qr_copro"&&<div style={{padding:"10px 14px",borderRadius:12,background:`${T.forest}08`,border:`1.5px solid ${T.forest}25`,marginBottom:12,display:"flex",alignItems:"center",gap:10,animation:"fadeIn 0.5s ease"}}>
          <span style={{fontSize:20}}>🏷</span>
          <div><div style={{fontSize:12,fontWeight:600,color:T.forest}}>QR code copropriété</div><div style={{fontSize:10,color:T.textMuted}}>Copropriété identifiée : <strong>{copro?.name}</strong>. Choisissez votre rôle pour continuer.</div></div>
        </div>}
...
function OnboardingConfirmResidence({copro,onConfirm,onWrong}) {
  ...
  return (
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:T.warmWhite,fontFamily:SANS}}>
      <div style={{padding:"56px 24px 20px",background:`linear-gradient(170deg,${T.forest},${T.forestLight})`,borderRadius:"0 0 28px 28px",flexShrink:0}}>
        <p style={{color:T.leafLight,fontSize:12,fontWeight:600,letterSpacing:"1.5px",textTransform:"uppercase",margin:"0 0 6px"}}>Confirmation</p>
        <h2 style={{fontFamily:FONT,fontSize:24,fontWeight:700,color:"#fff",margin:"0 0 6px"}}>Est-ce votre résidence ?</h2>
        <p style={{color:"rgba(255,255,255,0.7)",fontSize:13,margin:0}}>Vérifiez que vous habitez bien à cette adresse</p>
      </div>
        ...
        <Card style={{padding:20,border:`2px solid ${T.leafLight}`,boxShadow:`0 4px 24px ${T.forest}15`}}>
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:16}}>
            ...
            <div>
              ...
              <p style={{margin:"3px 0 0",fontSize:12,color:T.textLight}}>{copro?.label||copro?.city||""}</p>
            </div>
          </div>
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginTop:16}}>
            ...
            <button onClick={onWrong} style={{width:"100%",padding:12,borderRadius:12,border:`1.5px solid ${T.sandDark}`,background:"#fff",cursor:"pointer",fontFamily:SANS,fontSize:14,fontWeight:600,color:T.textLight,textAlign:"center"}}>✕ Non, ce n'est pas ici</button>
          </div>
        </Card>
        <div style={{marginTop:16,padding:"10px 14px",background:`${T.sunriseLight}15`,borderRadius:10}}>
          <p style={{fontSize:11,color:T.bark,margin:0,lineHeight:1.5}}>⚠️ Rejoindre une copropriété qui n'est pas la vôtre est contraire aux conditions d'utilisation. Votre identité sera vérifiée par un voisin.</p>
        </div>
      </div>
    </div>
  );
...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
function OnboardingProfileCreation({copro,role,isCS,onComplete,onCancel}) {
  const [show,setShow]=useState(false);
  ...
  return (
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:T.warmWhite,fontFamily:SANS}}>
      <div style={{padding:"56px 24px 18px",background:`linear-gradient(170deg,${T.forest},${T.forestLight})`,borderRadius:"0 0 28px 28px",flexShrink:0}}>
        <p style={{color:T.leafLight,fontSize:12,fontWeight:600,letterSpacing:"1.5px",textTransform:"uppercase",margin:"0 0 6px"}}>Dernière étape</p>
        <h2 style={{fontFamily:FONT,fontSize:24,fontWeight:700,color:"#fff",margin:"0 0 6px"}}>Créez votre profil</h2>
        <p style={{color:"rgba(255,255,255,0.7)",fontSize:13,margin:0}}>{copro?.name||"Ma Copropriété"}</p>
      </div>
      <div style={{padding:"20px 16px",flex:1,overflowY:"auto",opacity:show?1:0,transition:"opacity 0.5s 0.3s"}}>
        <Card style={{padding:16}}>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            ...
            <div style={{flex:1}}>
              <div style={{display:"flex",gap:4,flexWrap:"wrap"}}>
                ...
                {isCS&&<span style={{padding:"2px 7px",borderRadius:5,fontSize:9,fontWeight:700,background:`${T.sky}20`,color:T.sky}}>CS</span>}
                <span style={{padding:"2px 7px",borderRadius:5,fontSize:9,fontWeight:600,background:`${T.sunrise}15`,color:T.sunrise}}>N1 — Non vérifié</span>
              </div>
            </div>
          </div>
          ...
          <div style={{marginBottom:14}}>
            ...
            <input value={name} onChange={e=>setName(e.target.value)} placeholder="Prénom Nom" autoFocus style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`2px solid ${name.trim().length>=2?T.forest:T.sandDark}`,fontSize:15,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box",transition:"border-color 0.3s"}}/>
          </div>

          <div style={{marginBottom:14}}>
            <label style={{fontSize:10,fontWeight:600,color:T.textMuted,textTransform:"uppercase",letterSpacing:1,display:"block",marginBottom:4}}>Adresse email *</label>
            <input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="jean@exemple.fr" style={{width:"100%",padding:"12px 14px",borderRadius:10,border:`2px solid ${email.includes("@")?T.forest:T.sandDark}`,fontSize:15,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box",transition:"border-color 0.3s"}}/>
          </div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:10}}>
            <div style={{flex:2}}>
              ...
              <input value={building} onChange={e=>setBuilding(e.target.value)} placeholder="A, Mimosa..." style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`2px solid ${T.sandDark}`,fontSize:13,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box"}}/>
            </div>
            <div style={{flex:1}}>
              ...
              <input type="number" value={etage} onChange={e=>setEtage(e.target.value)} placeholder="3" style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`2px solid ${T.sandDark}`,fontSize:13,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box"}}/>
            </div>
            <div style={{flex:1.5}}>
              ...
              <input value={door} onChange={e=>setDoor(e.target.value)} placeholder="Gauche" maxLength={8} style={{width:"100%",padding:"10px 12px",borderRadius:10,border:`2px solid ${T.sandDark}`,fontSize:13,fontFamily:SANS,outline:"none",background:"#fff",color:T.text,boxSizing:"border-box"}}/>
            </div>
          </div>
      ...
      <div style={{padding:"12px 16px calc(12px + env(safe-area-inset-bottom, 8px))",background:T.warmWhite,borderTop:`1px solid ${T.sand}`,flexShrink:0}}>
        ...
        {onCancel&&<button onClick={onCancel} style={{width:"100%",marginTop:8,padding:10,background:"transparent",border:"none",cursor:"pointer",fontFamily:SANS,fontSize:13,color:T.textMuted,fontWeight:500,textAlign:"center"}}>← Revenir à l'accueil</button>}
      </div>
    </div>
  );
}
...
function MainApp({copro,role:initRole,isCS:initCS,isNew,waData,userApt,profileData,isAnonymous:initAnonymous,onCreateAccount}) {
  ...
  // ─── DEV CONTROLS ───
  const [devNiveau,setDevNiveau]=useState("1"); // "1" | "2" | "3"
  ...
  const [posts,setPosts]=useState(isNew?[
    {id:100,welcome:true,author:profileData?.name||"Jean Dupont",floor:"",time:"À l'instant",cat:"officiel",text:"",likedBy:[],role:initRole,replies:[],building:null},
  ...
  return (
    <div style={{height:"100%",background:T.sand,fontFamily:SANS,display:"flex",flexDirection:"column",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{background:`linear-gradient(135deg,${T.forest},${T.forestLight})`,padding:"48px 16px 12px",display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative",flexShrink:0,zIndex:20}}>
        <div style={{display:"flex",alignItems:"center",gap:10,flex:1,minWidth:0}}>
          <button onClick={()=>{if(isAnonymous){onCreateAccount&&onCreateAccount()}else{setShowProfile(true)}}} style={{background:"none",border:"2px solid rgba(255,255,255,0.3)",cursor:"pointer",padding:0,borderRadius:14,flexShrink:0}}>
            {profilePhoto
              ...
              :<div style={{width:36,height:36,borderRadius:12,background:isAnonymous?`linear-gradient(135deg,${T.sky},${T.purple})`:`linear-gradient(135deg,${T.sunrise},${T.coral})`,display:"flex",alignItems:"center",justifyContent:"center",color:"#fff",fontWeight:700,fontSize:isAnonymous?16:14,fontFamily:SANS}}>{isAnonymous?"👤":userName.split(" ").map(n=>n[0]).join("")}</div>
            }
          </button>
          <button onClick={()=>setShowCoproInfo(true)} style={{background:"none",border:"none",cursor:"pointer",padding:0,minWidth:0,textAlign:"left"}}>
            <p style={{fontSize:12,color:"#F5D49A",margin:"0 0 1px",fontWeight:700,letterSpacing:0.3}}>{isAnonymous?"Bienvenue !":`Bonjour ${userName.split(" ")[0]} !`}</p>
            ...
            <p style={{fontSize:10,color:"rgba(255,255,255,0.65)",margin:"1px 0 0"}}>{activeCopro.members} voisin{activeCopro.members>1?"s":""} · {activeCopro.logements} logements · {Math.round(activeCopro.members/activeCopro.logements*100)}%{isRef?" · ⭐ Référent":""}</p>
          </button>
        ...
        {!isAnonymous&&<button onClick={()=>setShowInvite(true)} style={{padding:"7px 10px",borderRadius:10,border:"1.5px solid rgba(255,255,255,0.25)",background:"rgba(255,255,255,0.08)",color:"#fff",fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:SANS,flexShrink:0}}>+ Inviter</button>}
      ...
      {isAnonymous&&<div onClick={()=>onCreateAccount&&onCreateAccount()} style={{padding:"10px 16px",background:`linear-gradient(90deg,${T.forest},${T.forestLight})`,display:"flex",alignItems:"center",justifyContent:"center",gap:8,cursor:"pointer",flexShrink:0}}>
        ...
        <span style={{fontSize:13,fontWeight:700,color:"#fff"}}>Créer un compte ou me connecter</span>
      ...
      <div style={{flex:1,overflowY:"auto",minHeight:0}}>
        ...
        {tab==="home"&&<div style={{padding:14}}>
          ...
          <div style={{display:"flex",gap:4,marginBottom:10,flexWrap:"wrap"}}>
            {[
              ...
              <span key={i} style={{padding:"2px 8px",borderRadius:6,fontSize:9,fontWeight:700,background:`${b.c}15`,color:b.c,border:`1px solid ${b.c}25`}}>{b.l}</span>
            ))}
          </div>
          ...
          {showProfileCompletion&&(
            <Card style={{padding:16,background:`linear-gradient(135deg,${T.leafLight}15,${T.sky}08)`,border:`1.5px solid ${T.leafLight}44`,marginBottom:12}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:10}}>
                ...
                <div>
                  <div style={{fontSize:15,fontWeight:700,color:T.forest}}>Bienvenue {userName.split(" ")[0]} !</div>
                  <div style={{fontSize:11,color:T.textMuted}}>Votre profil est créé. Voici vos premières étapes :</div>
                </div>
              </div>
              {[
                ...
                <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:i<3?`1px solid ${T.sand}`:"none"}}>
                  ...
                  <span style={{fontSize:12,color:s.done?T.textMuted:T.text,textDecoration:s.done?"line-through":"none"}}>{s.text}</span>
                </div>
              ))}
            </Card>
          )}
          ...
          {showGetVerified&&(
            <div onClick={()=>setShowVerifyGate("proprio")} style={{display:"flex",alignItems:"center",gap:10,padding:"12px 14px",background:`${T.sunrise}08`,borderRadius:14,marginBottom:12,cursor:"pointer",border:`1.5px solid ${T.sunrise}30`}}>
              <span style={{fontSize:18}}>🔓</span>
              <div style={{flex:1}}>
                <div style={{fontSize:13,fontWeight:600,color:T.bark}}>Faites vérifier votre identité</div>
                <div style={{fontSize:11,color:T.textMuted}}>Demandez à un voisin vérifié (N2+) de confirmer qui vous êtes</div>
              </div>
          ...
          {isRef&&<div style={{display:"flex",alignItems:"center",gap:10,padding:"10px 14px",background:`${T.forestLight}08`,borderRadius:14,marginBottom:12,border:`1.5px solid ${T.forestLight}25`}}>
            <span style={{fontSize:18}}>⭐</span>
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:600,color:T.forestLight}}>Vous êtes Référent de la copropriété</div>
              <div style={{fontSize:11,color:T.textMuted}}>Modération, vérification, accompagnement des nouveaux</div>
            </div>
          </div>}
          ...
          <Card style={{padding:14}}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:8}}>
              ...
              {!isAnonymous&&<button onClick={()=>setShowInvite(true)} style={{padding:"6px 12px",borderRadius:10,border:"none",background:`linear-gradient(135deg,${T.sunrise},${T.coral})`,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:SANS,flexShrink:0,whiteSpace:"nowrap"}}>+ Inviter</button>}
            </div>
          </Card>
          ...
          {canSeeVerifRequests&&!isNew&&activeCopro.members>=5&&(
            <div onClick={()=>setShowCadenas("verification")} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",background:`linear-gradient(135deg,${T.sunrise}08,${T.coral}08)`,borderRadius:14,marginBottom:10,cursor:"pointer",border:`1.5px solid ${T.sunrise}33`}}>
              <span style={{fontSize:18}}>🔒</span>
              <div style={{flex:1}}>
                <div style={{fontSize:12,fontWeight:600,color:T.bark}}>Voisins en attente de vérification</div>
                <div style={{fontSize:10,color:T.textMuted}}>{isGratuit?"Débloquez la vérification avec Essentiel":"Vérifiez leurs profils"}</div>
              </div>
              <span style={{fontSize:10,fontWeight:700,color:T.sunrise}}>{isGratuit?"Découvrir →":"Vérifier →"}</span>
            </div>
          )}
          ...
          {isNouveau&&(
            <Card style={{marginTop:10,padding:16,background:`${T.sky}06`,border:`1px solid ${T.sky}20`}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                <div>
                  <div style={{fontSize:14,fontWeight:600,color:T.text}}>Posez votre première question</div>
                  <div style={{fontSize:11,color:T.textMuted,lineHeight:1.4}}>Le Conseil AI peut vous renseigner sur vos droits, le règlement, les charges...</div>
                </div>
              </div>
              <button onClick={()=>setTab("ai")} style={{width:"100%",marginTop:10,padding:10,borderRadius:10,border:"none",background:`linear-gradient(135deg,${T.purple},${T.sky})`,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:SANS}}>💡 Essayer le Conseil AI</button>
            </Card>
          )}
        ...
        {tab==="feed"&&<div style={{padding:"14px 14px 70px"}}>
          ...
          {isAnonymous?
            <div onClick={()=>onCreateAccount&&onCreateAccount()} style={{display:"flex",gap:8,marginBottom:14,cursor:"pointer"}}>
              <div style={{flex:1,padding:"12px 16px",borderRadius:14,border:`1.5px dashed ${T.forest}40`,background:`${T.forest}06`,display:"flex",alignItems:"center",gap:10,fontFamily:SANS}}>
                <span style={{fontSize:16}}>🔒</span>
                <span style={{fontSize:12,color:T.forestLight,fontWeight:600}}>Créer un compte pour publier</span>
              </div>
            </div>
          ...
          {filteredPosts.map(p=>{
            ...
            <Card key={p.id}>
              ...
              <p style={{fontSize:13.5,color:T.text,lineHeight:1.6,margin:"0 0 8px",filter:isAnonymous?"blur(3px)":"none"}}>{p.text}</p>
        ...
        {tab==="msg"&&<div style={{padding:14,position:"relative"}}>
          {isAnonymous&&<div style={{position:"absolute",inset:0,zIndex:5,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:40,pointerEvents:"auto"}} onClick={()=>onCreateAccount&&onCreateAccount()}>
            <div style={{background:"#fff",borderRadius:16,padding:"20px 24px",boxShadow:"0 8px 32px rgba(0,0,0,0.15)",textAlign:"center",maxWidth:260,zIndex:6,cursor:"pointer"}}>
              <span style={{fontSize:28}}>🔒</span>
              <div style={{fontSize:14,fontWeight:700,color:T.forest,marginTop:6}}>Messages</div>
              <div style={{fontSize:12,color:T.textMuted,marginTop:4,lineHeight:1.5}}>Créez un compte ou connectez-vous pour envoyer des messages</div>
            </div>
          </div>}
          <div style={{filter:isAnonymous?"blur(4px)":"none",opacity:isAnonymous?0.4:1,pointerEvents:isAnonymous?"none":"auto"}}>
          {msgView==="list"?<div>
            {/* Action buttons */}
          ...
          <div style={{flex:1,overflowY:"auto",padding:14}}>
            ...
            {aiMsgs.length===1&&<div style={{display:"flex",flexWrap:"wrap",gap:7,marginTop:8}}>
              {["Mon voisin fait du bruit le dimanche","Règles pour les animaux ?","Comment contester une charge ?","Puis-je installer une clim sur mon balcon ?"].map((q,i)=>(
                <button key={i} onClick={()=>{if(isAnonymous){onCreateAccount&&onCreateAccount()}else{setAiQuery(q)}}} style={{padding:"8px 12px",borderRadius:10,border:`1px solid ${T.sandDark}`,background:"#fff",fontSize:12,color:T.text,cursor:"pointer",fontFamily:SANS}}>{q}</button>
              ))}
              {userDocs.length>0
          ...
          <div style={{padding:"10px 14px",background:T.warmWhite,borderTop:`1px solid ${T.sandDark}`,display:"flex",gap:8}}>
            {isAnonymous?
              <button onClick={()=>onCreateAccount&&onCreateAccount()} style={{flex:1,padding:"10px 14px",borderRadius:12,border:`1.5px dashed ${T.forest}40`,background:`${T.forest}06`,cursor:"pointer",fontFamily:SANS,display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:14}}>🔒</span>
                <span style={{fontSize:12,color:T.forestLight,fontWeight:600}}>Créer un compte ou me connecter pour poser une question</span>
              </button>
            :<>
        ...
        {tab==="copro"&&<div style={{padding:"0 14px 14px"}}>
          ...
          {agendaTab==="docs"&&<div style={{position:"relative"}}>
            ...
            {isGratuit&&<div style={{position:"absolute",inset:0,zIndex:5,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"auto"}}>
              <div style={{background:"#fff",borderRadius:18,padding:"24px 28px",boxShadow:"0 8px 32px rgba(0,0,0,0.15)",textAlign:"center",maxWidth:280,zIndex:6}}>
                ...
                <div style={{fontSize:12,color:T.textMuted,marginTop:4,lineHeight:1.5}}>Disponible pour les versions <strong>Essentiel</strong> et <strong>Premium</strong></div>
                <button onClick={()=>setShowCadenas("conseil")} style={{marginTop:14,padding:"10px 24px",borderRadius:10,border:"none",background:`linear-gradient(135deg,${T.purple},${T.sky})`,color:"#fff",fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:SANS}}>Découvrir les plans</button>
              </div>
            </div>}
              ...
              {(()=>{const st=SUJET_STATUS[sujetDetail.status];const cat=SUJET_CAT[sujetDetail.category];return(<div>
                ...
                {sujetDetail.status!=="resolu"&&sujetDetail.aiSugg.length>0&&<Card style={{border:`1px solid ${isGratuit?T.sandDark:T.forestLight+"30"}`,background:isGratuit?`${T.sand}44`:`linear-gradient(135deg,${T.forest}04,${T.leafLight}10)`,padding:12,position:"relative"}}>
                  {isGratuit&&<div style={{position:"absolute",inset:0,borderRadius:18,background:"rgba(255,255,255,0.6)",zIndex:2,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    <div style={{textAlign:"center",padding:16}}>
                      <span style={{fontSize:28}}>🔒</span>
                      <div style={{fontSize:12,fontWeight:700,color:T.purple,marginTop:4}}>Actions AI — Essentiel</div>
                      <div style={{fontSize:10,color:T.textMuted,marginTop:2}}>Disponible avec le plan Essentiel ou Premium</div>
                    </div>
                  </div>}
                  <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><div style={{width:24,height:24,borderRadius:6,background:`linear-gradient(135deg,${T.forest},${T.forestLight})`,display:"flex",alignItems:"center",justifyContent:"center"}}><span style={{fontSize:12}}>🤖</span></div><h4 style={{margin:0,fontSize:11,fontWeight:700,color:T.forest}}>Actions IA suggérées</h4>{isGratuit&&<span style={{fontSize:10}}>🔒</span>}</div>
                  {sujetDetail.aiSugg.map((sug,i)=>(
                    <div key={i} style={{marginBottom:5}}>
                      <button onClick={()=>{if(!isGratuit){setSujetAiIdx(sujetAiIdx===i?null:i);setSujetAiResult(null)}}} style={{width:"100%",padding:"7px 10px",borderRadius:8,border:sujetAiIdx===i&&!isGratuit?`1.5px solid ${T.forest}`:`1px solid ${T.sandDark}`,background:sujetAiIdx===i&&!isGratuit?`${T.leafLight}18`:"#fff",cursor:isGratuit?"default":"pointer",textAlign:"left",fontFamily:SANS,display:"flex",alignItems:"center",gap:6,opacity:isGratuit?0.6:1}}>
                        ...
                        {isGratuit?<span style={{fontSize:10}}>🔒</span>:<span style={{fontSize:10,color:T.forestLight}}>{sujetAiIdx===i?"▲":"▼"}</span>}
                      </button>
                ...
                {sujetDetail.status!=="resolu"&&(isGratuit?
                  <Card style={{marginTop:4,padding:10,opacity:0.5}}>
                    <div style={{display:"flex",alignItems:"center",gap:6}}><span style={{fontSize:14}}>🔒</span><span style={{fontSize:11,color:T.textMuted}}>Ajouter un signalement — Essentiel</span></div>
                  </Card>
          ...
          {agendaTab==="gouv"&&<div>
            ...
            {isN1&&<div style={{padding:"10px 14px",background:`${T.sunrise}08`,borderRadius:12,marginBottom:10,display:"flex",alignItems:"center",gap:8,border:`1.5px solid ${T.sunrise}25`}}>
              <span style={{fontSize:16}}>🔒</span>
              <div><div style={{fontSize:12,fontWeight:600,color:T.bark}}>Profil non vérifié (N1)</div><div style={{fontSize:10,color:T.textMuted}}>Vous pouvez consulter le tableau de bord. Les outils de gouvernance nécessitent un statut N2+.</div></div>
            </div>}
            {isN2&&!isN3&&<div style={{padding:"10px 14px",background:`${T.sky}08`,borderRadius:12,marginBottom:10,display:"flex",alignItems:"center",gap:8,border:`1.5px solid ${T.sky}25`}}>
              <span style={{fontSize:16}}>👁</span>
              <div><div style={{fontSize:12,fontWeight:600,color:T.sky}}>Consultation seule (N2)</div><div style={{fontSize:10,color:T.textMuted}}>Vous pouvez consulter et vérifier des voisins. La modification des paramètres nécessite N3 (vérifié par le syndic){isGratuit?" + plan Essentiel":""}.</div></div>
            </div>}
            {isN3&&isGratuit&&<div onClick={()=>setShowCadenas("conseil")} style={{padding:"10px 14px",background:`${T.purple}06`,borderRadius:12,marginBottom:10,display:"flex",alignItems:"center",gap:8,border:`1.5px solid ${T.purple}20`,cursor:"pointer"}}>
              <span style={{fontSize:16}}>✨</span>
              <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:T.purple}}>Plan Gratuit — consultation seule</div><div style={{fontSize:10,color:T.textMuted}}>Paramètres, référents et consultations modifiables avec le plan Essentiel.</div></div>
              <span style={{fontSize:10,fontWeight:700,color:T.purple}}>Découvrir →</span>
            </div>}
            {canEditGov&&<div style={{padding:"6px 10px",background:`${T.forest}08`,borderRadius:8,marginBottom:10,display:"flex",alignItems:"center",gap:6}}>
              ...
              <span style={{fontSize:10,fontWeight:600,color:T.forest}}>Accès complet — N3 + {isPremium?"Premium":"Essentiel"}</span>
            </div>}
            ...
            {govView==="dashboard"&&<div>
              ...
              <Card style={{padding:12}}>
                ...
                {pendingVerifs.length>0&&<div onClick={()=>{if(!isN1)setGovView("verification")}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${T.sand}`,cursor:isN1?"default":"pointer",opacity:isN1?0.5:1}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:4}}>
                    ...
                    {isN1&&<span style={{fontSize:8,color:T.textMuted}}>🔒 N2+</span>}
                  </div>
                </div>}
                {departures.filter(d=>d.status==="pending").length>0&&<div onClick={()=>{if(!isN1&&!isNouveau)setGovView("departures")}} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",cursor:(isN1||isNouveau)?"default":"pointer",opacity:(isN1||isNouveau)?0.5:1}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:4}}>
                    ...
                    {(isN1||isNouveau)&&<span style={{fontSize:8,color:T.textMuted}}>🔒</span>}
                  </div>
                </div>}
                ...
                {isN1&&<div style={{marginTop:6,padding:"6px 10px",borderRadius:8,background:`${T.sunrise}06`}}>
                  <p style={{fontSize:9,color:T.bark,margin:0}}>🔒 Faites vérifier votre profil (N2+) pour accéder aux actions de gouvernance.</p>
                </div>}
              </Card>
              ...
              <Card style={{padding:12}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                  ...
                  {!isN1&&<button onClick={()=>setGovView("referents")} style={{fontSize:10,color:canEditGov?T.forest:T.textMuted,fontWeight:600,background:"none",border:"none",cursor:"pointer",fontFamily:SANS}}>{canEditGov?"Gérer →":"Voir →"}</button>}
                  {isN1&&<span style={{fontSize:9,color:T.textMuted}}>🔒 N2+</span>}
                </div>
              ...
              <Card style={{padding:12}}>
                <h4 style={{margin:"0 0 8px",fontSize:12,fontWeight:700,color:T.text}}>Profil de gouvernance {!canEditGov&&<span style={{fontSize:9,fontWeight:500,color:T.textMuted}}>(lecture seule)</span>}</h4>
                ...
                {!canEditGov&&<div style={{marginTop:6,fontSize:9,color:T.textMuted}}>🔒 {isN1?"Niveau N2+ requis":isGratuit?"Plan Essentiel requis":!isN3?"Niveau N3 requis":"—"}</div>}
              </Card>
            </div>}
            ...
            {govView==="referents"&&<div>
              ...
              {!canEditGov&&<div style={{padding:"8px 12px",background:`${T.sunrise}08`,borderRadius:10,marginBottom:10,border:`1px solid ${T.sunrise}20`}}><p style={{fontSize:10,color:T.bark,margin:0}}>🔒 {!isN3?"Niveau N3":"Plan Essentiel"} requis pour gérer les référents.</p></div>}
              <Card style={{padding:12}}>
                ...
                {referents.map((r,i)=>(
                  <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 0",borderBottom:i<referents.length-1?`1px solid ${T.sand}`:"none"}}>
                    ...
                    {canEditGov&&<button onClick={()=>setReferents(p=>p.filter((_,j)=>j!==i))} style={{padding:"4px 10px",fontSize:10,borderRadius:6,border:`1px solid ${T.sandDark}`,background:"#fff",color:T.textMuted,cursor:"pointer",fontFamily:SANS}}>Retirer</button>}
                  </div>
                ))}
              </Card>
            ...
            {govView==="settings"&&<div>
              {!canEditGov&&<div style={{padding:"12px 16px",background:`${T.purple}08`,borderRadius:12,marginBottom:10,textAlign:"center",border:`1.5px dashed ${T.purple}30`}}>
                ...
                <div style={{fontSize:13,fontWeight:600,color:T.purple,marginTop:4}}>Paramètres verrouillés</div>
                <div style={{fontSize:11,color:T.textMuted,marginTop:2}}>{!isN3?"Niveau N3 (vérifié par le syndic)":"Plan Essentiel"} requis pour modifier les paramètres de gouvernance.</div>
                {isGratuit&&<button onClick={()=>setShowCadenas("conseil")} style={{marginTop:8,padding:"7px 18px",borderRadius:8,border:"none",background:`linear-gradient(135deg,${T.purple},${T.sky})`,color:"#fff",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:SANS}}>Découvrir Essentiel</button>}
              </div>}
            ...
            {govView==="consultations"&&<div>
              {!canEditGov&&<div style={{padding:"8px 12px",background:`${T.sunrise}08`,borderRadius:10,marginBottom:10,border:`1px solid ${T.sunrise}20`}}><p style={{fontSize:10,color:T.bark,margin:0}}>🔒 {!isN3?"Niveau N3":"Plan Essentiel"} requis pour lancer ou modifier des consultations. Consultation en lecture seule.</p></div>}
              ...
              {canEditGov&&(!showNewConsult?<button onClick={()=>setShowNewConsult(true)} style={{width:"100%",padding:10,borderRadius:12,border:`2px dashed ${T.purple}40`,background:"transparent",cursor:"pointer",fontFamily:SANS,fontSize:12,fontWeight:600,color:T.purple,marginTop:4}}>+ Lancer une consultation</button>
      ...
      {showAnnuaire&&<div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"fadeIn 0.3s"}} onClick={()=>{setShowAnnuaire(false);setAnnuaireSearch("");setSyndicRevealed(false)}}>
        <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:420,background:T.warmWhite,borderRadius:"22px 22px 0 0",padding:"8px 18px 28px",maxHeight:"85vh",overflowY:"auto",animation:"slideUp 0.4s cubic-bezier(0.16,1,0.3,1)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",margin:"4px 0 8px"}}>
            ...
            {(isCS||role==="syndic"||isNew)&&!showAddPrest&&<button onClick={()=>{setShowAddPrest(true);setAddPrestForm({name:"",type:"",tel:""})}} style={{background:"none",border:"none",cursor:"pointer",fontSize:10,fontWeight:600,color:T.forest,fontFamily:SANS}}>+ Ajouter</button>}
          </div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                {isSyndicEntry
                  ?<button onClick={()=>setSyndicRevealed(!syndicRevealed)} style={{background:"none",border:"none",cursor:"pointer",fontFamily:SANS}}>
                    {syndicRevealed
                      ?<span style={{fontSize:12,fontWeight:600,color:telEmpty?T.textMuted:T.forest}}>{p.tel}</span>
                      :<span style={{fontSize:11,fontWeight:600,color:T.coral}}>Voir le n°</span>
                    }
                  </button>
                  :!isEditing&&<span style={{fontSize:12,fontWeight:600,color:telEmpty?T.textMuted:isUrgence?T.red:T.forest}}>{telEmpty?"Non renseigné":p.tel}</span>
                }
                {(isCS||role==="syndic"||isNew)&&!isSyndicEntry&&!isUrgence&&!isEditing&&<button onClick={()=>{setEditPrestIdx(i);setEditPrestForm({name:p.name,type:p.type,tel:p.tel,email:p.email||""})}} style={{background:"none",border:"none",cursor:"pointer",fontSize:12,color:T.textMuted,padding:2}}>✏️</button>}
              </div>
              {/* Inline full edit */}
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",margin:"14px 0 8px"}}>
            ...
            {(isCS||role==="syndic"||isNew)&&!showAddInfo&&<button onClick={()=>{setShowAddInfo(true);setAddInfoForm({l:"",v:""})}} style={{background:"none",border:"none",cursor:"pointer",fontSize:10,fontWeight:600,color:T.forest,fontFamily:SANS}}>+ Ajouter</button>}
          </div>
          {/* Add info form */}
              ...
              <div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:i<infoPratiques.length-1?`1px solid ${T.sand}`:"none"}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4}}>
                  {editInfoIdx===i
                    ...
                    :info.isCode?<>
                      {showCodePorte
                        ?<span style={{fontSize:12,fontWeight:700,color:T.text,fontFamily:"monospace",letterSpacing:1}}>{info.v}</span>
                        :<button onClick={()=>setShowCodePorte(true)} style={{padding:"3px 10px",borderRadius:6,border:`1px solid ${T.forestLight}`,background:`${T.forest}08`,fontSize:10,fontWeight:600,color:T.forest,cursor:"pointer",fontFamily:SANS}}>👁 Voir le code</button>
                      }
                      {(isCS||role==="syndic"||isNew)&&<button onClick={()=>{setEditInfoIdx(i);setEditInfoVal(info.v)}} style={{background:"none",border:"none",cursor:"pointer",fontSize:9,color:T.textMuted,padding:0}}>✏️</button>}
                    </>
                    :<>
                      <span style={{fontSize:12,fontWeight:700,color:info.v==="—"?T.textMuted:T.text}}>{info.v}</span>
                      {(isCS||role==="syndic"||isNew)&&<button onClick={()=>{setEditInfoIdx(i);setEditInfoVal(info.v)}} style={{background:"none",border:"none",cursor:"pointer",fontSize:9,color:T.textMuted,padding:0}}>✏️</button>}
                    </>
                  }
                </div>
            ...
            return(<div style={{marginTop:14,position:"relative"}}>
              {isAnonymous&&<div style={{position:"absolute",inset:0,zIndex:3,display:"flex",alignItems:"center",justifyContent:"center",pointerEvents:"auto"}} onClick={()=>{setShowAnnuaire(false);onCreateAccount&&onCreateAccount()}}>
                <div style={{background:"#fff",borderRadius:14,padding:"16px 20px",boxShadow:"0 4px 20px rgba(0,0,0,0.12)",textAlign:"center",zIndex:4,cursor:"pointer"}}>
                  <span style={{fontSize:22}}>🔒</span>
                  <div style={{fontSize:12,fontWeight:700,color:T.forest,marginTop:4}}>Créer un compte pour voir les résidents</div>
                </div>
              </div>}
        ...
        <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:420,background:T.warmWhite,borderRadius:"22px 22px 0 0",maxHeight:"85vh",display:"flex",flexDirection:"column",overflow:"hidden",animation:"slideUp 0.4s cubic-bezier(0.16,1,0.3,1)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:14}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{marginTop:3,display:"flex",gap:4,flexWrap:"wrap"}}>
                ...
                {isRef&&<span style={{padding:"2px 7px",borderRadius:5,fontSize:9,fontWeight:700,background:`${T.forestLight}20`,color:T.forestLight}}>⭐ Référent</span>}
          ...
          {profTab==="profil"&&<div>
            ...
            <div style={{marginTop:16,padding:14,borderRadius:14,background:`${T.purple}06`,border:`1.5px dashed ${T.purple}30`}}>
              <div style={{fontSize:10,fontWeight:700,color:T.purple,textTransform:"uppercase",letterSpacing:1,marginBottom:10,display:"flex",alignItems:"center",gap:5}}>
                <span style={{fontSize:12}}>🧪</span> Contrôles développeur
              </div>
              {[
                ...
                <div key={i} style={{marginBottom:i<2?10:0}}>
                  <label style={{fontSize:9,fontWeight:600,color:T.purple,display:"block",marginBottom:3}}>{ctrl.label}</label>
                  <select value={ctrl.value} onChange={e=>ctrl.set(e.target.value)} style={{width:"100%",padding:"8px 10px",borderRadius:8,border:`1.5px solid ${T.purple}30`,fontSize:11,fontFamily:SANS,color:T.text,background:"#fff",cursor:"pointer",outline:"none"}}>
                    {ctrl.options.map(o=><option key={o.v} value={o.v}>{o.l}</option>)}
                  </select>
                </div>
              ))}
              <div style={{marginTop:8,padding:"6px 10px",borderRadius:8,background:`${T.purple}10`,fontSize:9,color:T.purple,lineHeight:1.5}}>
  ...
  return (
    <div style={{position:"relative",height:"100dvh",background:isMobile?"#e8e8e8":T.warmWhite,overflow:"hidden"}}>
      ...
      <div style={{
        ...
        {/* ─── SCENARIO SELECTOR ─── */}
        {screen==="welcome"&&<OnboardingWelcome onNext={(mode)=>{
          setEntryMode(mode);
        ...
        {screen==="profile_create"&&<OnboardingProfileCreation copro={copro} role={userRole} isCS={userIsCS} onComplete={(pd)=>{setProfileData(pd);setIsAnonymous(false);setScreen("app")}} onCancel={()=>{setIsAnonymous(true);setScreen("app")}}/>}

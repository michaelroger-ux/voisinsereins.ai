...
/* ═══════════════════════════════════════
   MAIN APP
   ═══════════════════════════════════════ */
function MainApp({copro,role,isCS,isNew}) {
  ...
  const handlePublish = () => {
    ...
    setPosts([{id:Date.now(),author:"Vous",floor:"2A",time:"À l'instant",text,cat:feedCat==="all"?"convivialité":feedCat,likes:0,role},... posts]);
  ...
  const TABS=[
    ...
    {id:"annonces",icon:"🤝",label:"Entraide"},
  ];
  ...
  return (
    <div style={{minHeight:"100vh",background:T.sand,fontFamily:SANS,display:"flex",flexDirection:"column"}}>
      ...
      <div style={{flex:1,overflowY:"auto",paddingBottom:76}}>
        ...
        {tab==="home"&&<div style={{padding:14}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18}}>
            {[
              ...
              {icon:"🤝",label:"Entraide",desc:"Petites annonces",color:T.coral,tab:"annonces"},
        ...
        {/* ═══ COPRO TAB (Agenda + Docs) ═══ */}
        {tab==="copro"&&<div style={{padding:14}}>
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            {[{id:"events",l:"📅 Agenda"},{id:"docs",l:"📁 Documents"}].map(t=>(
              <button key={t.id} onClick={()=>setAgendaTab(t.id)} style={{padding:"8px 16px",borderRadius:10,border:"none",background:agendaTab===t.id?T.forest:"#fff",color:agendaTab===t.id?"#fff":T.text,fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:SANS}}>{t.l}</button>
            ))}
          </div>
        ...
        {/* ═══ ENTRAIDE / ANNONCES TAB ═══ */}
        {tab==="annonces"&&<div style={{padding:14}}>
      ...
      {showProfile&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",zIndex:1000,display:"flex",alignItems:"flex-end",justifyContent:"center",animation:"fadeIn 0.3s"}} onClick={()=>setShowProfile(false)}>
        <div onClick={e=>e.stopPropagation()} style={{width:"100%",maxWidth:420,background:T.warmWhite,borderRadius:"22px 22px 0 0",padding:"8px 20px 36px",maxHeight:"85vh",overflowY:"auto",animation:"slideUp 0.4s cubic-bezier(0.16,1,0.3,1)"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:14,marginBottom:24}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{marginTop:4,display:"flex",gap:6}}>
                <span style={{padding:"2px 8px",borderRadius:6,fontSize:10,fontWeight:700,background:`${T.forest}15`,color:T.forest}}>{role==="proprio"?"Copropriétaire":role==="locataire"?"Locataire":"Syndic professionnel"}</span>
                {isCS&&<span style={{padding:"2px 8px",borderRadius:6,fontSize:10,fontWeight:700,background:`${T.sky}20`,color:T.sky}}>Conseil syndical</span>}
          ...
          {/* Role info */}
          <div style={{background:`${T.leafLight}18`,borderRadius:14,padding:14,marginTop:20,marginBottom:16}}>
            ...
            {role==="proprio"?
              <p style={{fontSize:12,color:T.textLight,margin:0,lineHeight:1.5}}>Accès complet : documents AG, votes, médiation, conseil juridique copropriété, tous les fils, messagerie, petites annonces.{isCS?" En tant que membre du conseil syndical, vous avez également accès aux outils de suivi CS.":""}</p>:
            role==="syndic"?
              <p style={{fontSize:12,color:T.textLight,margin:0,lineHeight:1.5}}>Dashboard de gestion, diffusion officielle, analytics d'engagement, ticketing, gestion des procurations et votes AG.</p>:
              <p style={{fontSize:12,color:T.textLight,margin:0,lineHeight:1.5}}>Fil d'actualité, petites annonces, messagerie, conseil AI (orienté bail). Pas d'accès aux documents financiers ni aux votes d'AG.</p>
            }
          </div>
